...
def play_sequence():
    ...
    for value in game_sequence:
        ...
        #Increment delay for the next iteration of loop
        delay += time
...
def change_label(message):
    """Update the start button text and color to let the player know they are status."""
...
def set_difficulty():
    ...
    #Change the time (difficulty)  based off the value of the radio button
...
def press(value):
    """Simulate pressing a button for a player"""
    #Add the players press to player sequence
    player_sequence.append(value)

    #if the current round is over check to see player entered correct sequence
    if len(player_sequence) == len(game_sequence):
        check_round()

def check_round():
    """Determine ifthe player entered the correct sequence of button presses for a round"""
    ...
    #The player is incorrect so change label, update score, disable buttons, and reset for new game
        ...
        game_sequence = []
        #Wait 2 sec then change the message
        root.after(2000, lambda:change_label("New Game"))

    #Regardless of win or lose for the round, wipe the player sequence
    player_sequence = []
...
white_button = Button(game_frame, bg=white, activebackground=white_light, state=DISABLED, borderwidth=3, command=lambda:press(1))
magenta_button = Button(game_frame, bg=magenta, activebackground=magenta_light, state=DISABLED, borderwidth=3, command=lambda:press(2))
cyan_button = Button(game_frame, bg=cyan, activebackground=cyan_light, state=DISABLED, borderwidth=3, command=lambda:press(3))
yellow_button = Button(game_frame, bg=yellow, activebackground=yellow_light, state=DISABLED, borderwidth=3, command=lambda:press(4))
...
Radiobutton(game_frame, text='Easy', variable=difficulty, value='Easy', font=game_font2, bg=game_color, command=set_difficulty).grid(row=2, column=1)
Radiobutton(game_frame, text='Medium', variable=difficulty, value='Medium', font=game_font2, bg=game_color, command=set_difficulty).grid(row=2, column=2)
Radiobutton(game_frame, text='Hard', variable=difficulty, value='Hard', font=game_font2, bg=game_color, command=set_difficulty).grid(row=2, column=3)
